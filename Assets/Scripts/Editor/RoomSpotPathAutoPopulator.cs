...
public class RoomSpotPathAutoPopulator : Editor
{
    ...
    private static void AutoPopulate(RoomBase room)
    {
        ...
        List<RoomPath> newPaths = new List<RoomPath>();
        ...
        pathsField.SetValue(room, newPaths);

        EditorUtility.SetDirty(room);
        PrefabUtility.RecordPrefabInstancePropertyModifications(room);

        string summary = $"{room.name}: populated {spotsUsedForPaths.Count} spot(s) across matched field(s), and regenerated {newPaths.Count} Spot Path(s) (waypoints left empty for hand-routing).";
