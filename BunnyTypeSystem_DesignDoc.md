...
- **Egg/breeding path (kid bunnies, 1 trait, level-up gaining new traits) doesn't exist yet.** `RollTraits`/`ResolvePassives` are written generically enough to support it (`RollTraits(1)` for a kid, a future `LevelUp()` re-running `ResolvePassives` and pushing a new trait roll), but nothing calls them that way today — `PopulationManager.ResidentCategory.Egg` exists as a bucket but has no hatch logic anywhere in the codebase yet.
