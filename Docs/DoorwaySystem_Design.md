...
- **Entrance room's outward-facing edge** — never has a neighbor by construction (nothing can be built beyond it), so it naturally stays sealed forever without needing to special-case "this is the entrance." The other side behaves like any other room's edge.
